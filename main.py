...
app2.layout = html.Div([
    ...
    html.Div(id='audio-player', style={'textAlign': 'center'}),
    html.Div(id='sec-audio-player', style={'textAlign': 'center'}),
    dcc.Graph(
        id='spectrogram',
        config={
            'modeBarButtonsToAdd': ['drawrect', 'eraseshape', 'zoom', 'zoomIn', 'zoomOut', 'resetScale2d'],
    ...
    html.Div(id='sec-clipped-audio-player', style={'textAlign': 'center'}),
...
@app2.callback(
    [Output('audio-player', 'children'),
     ...
     Output('sec-audio-player', 'children'),
     Output('sec-clipped-audio-player', 'children')],
    [Input('upload-audio', 'contents'),
...
def update_output(contents, move_clicks, n_intervals, confirm_clicks, relayoutData, line_position, audio_content, start_time, start_time_input, end_time_input, clip_time):
    ...
    if not ctx.triggered:
        return '', go.Figure(), True, line_position, no_update, no_update, start_time, clip_time, no_update, no_update, no_update, no_update, no_update
    ...
    if trigger_id == 'upload-audio' and contents:
        ...
        fig = create_spectrogram_figure(y, sr, duration, line_position, 'Spectrogram of audio signal')

        audio_src = f'data:audio/wav;base64,{contents.split(",")[1]}'
        audio_player = html.Audio(src=audio_src, controls=True, id='audio-element', style={'display': 'none'})
        audio_player_visible = html.Audio(src=audio_src, controls=True, id='audio-element', style={'width': '30%'})

        return audio_player, fig, True, line_position, no_update, contents, {'start': None}, {'start': 0, 'end': duration}, no_update, no_update, no_update, audio_player_visible, no_update

    elif trigger_id == 'move-line-button' and move_clicks:
        start_time = {'start': datetime.now().timestamp()}
        return no_update, no_update, False, {'x': 0, 'moving': True}, no_update, audio_content, start_time, clip_time, no_update, no_update, no_update, no_update, no_update
        ...
        return no_update, fig, not line_position['moving'], line_position, no_update, no_update, start_time, clip_time, no_update, no_update, no_update, no_update, no_update
        ...
        fig = create_spectrogram_figure(y, sr, len(y) / sr, line_position, 'Spectrogram of audio signal')
        ...
        clipped_audio_player = html.Audio(src=f'data:audio/wav;base64,{clipped_audio_b64}', controls=True, id='clipped-audio-element', style={'display': 'none'})
        clipped_audio_player_visible = html.Audio(src=f'data:audio/wav;base64,{clipped_audio_b64}', controls=True, id='clipped-audio-element', style={'width': '30%'})

        return no_update, fig, no_update, line_position, no_update, audio_content, start_time, clip_time, clipped_audio_player, no_update, no_update, no_update, clipped_audio_player_visible
        ...
        return no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update, start_time_input, end_time_input, no_update, no_update

    return no_update, no_update, True, line_position, no_update, no_update, start_time, clip_time, no_update, no_update, no_update, no_update, no_update
